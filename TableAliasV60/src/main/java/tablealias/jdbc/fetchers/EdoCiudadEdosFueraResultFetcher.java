 ...
 * @author INEGI
 */
public class EdoCiudadEdosFueraResultFetcher implements QueryFetcher{

    private EdoCiudadData dataToFill;

    public EdoCiudadEdosFueraResultFetcher(EdoCiudadData dataToFill) {
        this.dataToFill = dataToFill;
    }


    public Object fetchResults(ResultSet rs) throws SQLException {
        List<Estados> edos = dataToFill.getEstadosFuera();
        while(rs.next()){
